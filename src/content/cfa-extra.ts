...
export const extraQuestions: Question[] = [
  ...
  },
  {
    ...
    choices: ["Large economic profits from pricing power", "Persistent economic losses", "Normal profits (zero economic profit)"],
    answerIndex: 2,
    explanation: "Under perfect competition, free entry and exit drive long-run economic profit to zero — firms earn only a 'normal' profit (their cost of capital). If profits were positive, new entrants would compete them away; if negative, firms would exit until the survivors break even. Choice A describes a monopoly or differentiated market with pricing power, which perfect competition lacks. Choice B can't persist because loss-making firms exit, restoring equilibrium.",
  },
  // Corporate Issuers
  {
    ...
    choices: ["Choose the higher-NPV project", "Choose the higher-IRR project", "Reject both projects"],
    answerIndex: 0,
    explanation: "When NPV and IRR conflict for mutually exclusive projects, NPV is the better criterion because it measures the actual dollar value created, while IRR (a percentage) can favor a smaller project that adds less total value. So the firm picks the higher-NPV project. Choice B blindly follows IRR and can leave value on the table. Choice C is wrong because a positive NPV project creates value and shouldn't be rejected.",
  },
  {
  ...
  },
  {
    ...
    choices: ["Less volatile across the business cycle", "Equally volatile, since sales variability is the same", "More volatile across the business cycle"],
    answerIndex: 2,
    explanation: "Leverage magnifies how sales changes flow through to net income. High operating leverage amplifies the effect of sales swings on operating profit, and high financial leverage further amplifies the effect on net income through fixed interest costs. So Firm X's net income swings much more than Firm Y's. Choice B is the trap: identical sales variability does NOT mean identical earnings variability, because leverage multiplies it.",
  },
  // Equity
  {
  ...
  },
  {
    ...
    choices: ["The highest-priced stocks", "All constituents equally", "The largest companies by market value"],
    answerIndex: 2,
    explanation: "In a market-cap-weighted index, each constituent's influence is proportional to its total market value, so the largest companies dominate the index's movements. Choice A describes a PRICE-weighted index (like the Dow), where share price, not company size, drives weight. Choice B describes an equal-weighted index. Knowing the weighting scheme is essential before comparing performance to a benchmark.",
  },
  // Derivatives
  {
    ...
    choices: ["Standardized, exchange-traded, and marked to market daily", "Customized and carry higher counterparty risk", "Always more expensive to transact than an equivalent forward contract"],
    answerIndex: 0,
    explanation: "Futures are standardized, traded on exchanges, and marked to market daily through a clearinghouse, which largely eliminates counterparty risk. Choice B describes forwards, which are customized OTC contracts with real counterparty (default) risk. Choice C is simply false — there's no general rule that futures cost more; their prices track the same no-arbitrage relationship as forwards.",
  },
  {
  ...
  },
  {
    ...
    choices: ["Two puts", "The underlying minus a put", "A put plus the underlying asset"],
    answerIndex: 2,
    explanation: "Put-call parity: call + bond = put + underlying. The left and right sides produce identical payoffs at expiration, so by no-arbitrage they must have equal value. Choice B (underlying − put) equals a different combination and isn't the parity identity. Choice A is unrelated. Parity lets you build any one instrument synthetically from the others — a frequently tested idea.",
  },
  // Alternatives
  {
    ...
    choices: ["Improve diversification through lower correlation with traditional assets", "Guarantee consistently higher returns while carrying materially less risk", "Reduce the management fees paid across the overall portfolio"],
    answerIndex: 0,
    explanation: "Alternatives are added mainly because they tend to be less correlated with stocks and bonds, so they can improve a portfolio's overall risk-adjusted return even when volatile individually. Choice B overpromises — nothing guarantees higher returns with less risk, and alternatives are often riskier standalone. Choice C is backwards: alternatives typically carry HIGHER fees (e.g., '2 and 20'), not lower.",
  },
  {
    ...
    choices: ["Earns the 2% management fee only during years the fund is profitable", "Earns performance fees only on net new profits above the prior peak", "Never charges any management fee in a year following an investment loss"],
    answerIndex: 1,
    explanation: "A high-water mark means the 20% performance fee applies only to gains above the fund's previous highest value — so after a loss, the manager must first recover that loss before earning performance fees again, preventing them from being paid twice for the same gains. Choices B and C confuse this with the 2% MANAGEMENT fee, which is charged on assets regardless of performance; the high-water mark applies to the performance fee, not the management fee.",
  },
  {
    ...
    choices: ["Overstate true volatility", "Perfectly reflect true volatility", "Understate true volatility"],
    answerIndex: 2,
  ...
  // Portfolio Management
  {
    ...
    choices: ["Systematic risk only", "Total risk (systematic + unsystematic)", "Unsystematic risk only"],
    answerIndex: 0,
    explanation: "Only systematic (market) risk is rewarded, because unsystematic (firm-specific) risk can be eliminated through diversification at no cost — the market won't pay a premium for a risk you could have avoided. Choice B is the common error of assuming total volatility earns a premium. Choice C is backwards: unsystematic risk is exactly the part that earns NO premium because it's diversifiable.",
  },
  {
