...
export const deepQuestionsL2: Question[] = [
  {
    ...
    choices: ["Holding the other independent variables fixed", "Ignoring every other variable in the model", "Only if X₁ is significant", "Only when evaluated at the mean of Y"],
    answerIndex: 0,
  ...
  },
  {
    ...
    stem: "An analyst finds conditional heteroskedasticity in a regression but leaves it uncorrected. What is the most direct consequence for the analysis?",
    choices: ["Coefficient estimates become biased and must be re-estimated", "The R-squared statistic becomes meaningless as a fit measure", "The regression's residuals are no longer required to sum to zero", "Standard errors are unreliable, so t-tests may reach false conclusions"],
    answerIndex: 3,
    explanation: "Conditional heteroskedasticity leaves coefficients unbiased but corrupts the standard errors, typically understating them and thereby inflating t-statistics. The practical result is finding significance where none exists - a Type I error. The remedy is robust standard errors rather than re-estimating the coefficients. R-squared remains interpretable as a fit measure, and the residuals still sum to zero by construction of OLS.",
  },
  {
    id: "cfa-l2reg-d5", examSlug: "cfa-l2", topicId: "quant", topicName: "Quantitative Methods", difficulty: 2,
    stem: "A regression on quarterly data produces a Durbin-Watson statistic of 1.98. What does this indicate about the residuals?",
    choices: ["Essentially no first-order serial correlation is present", "Strong positive serial correlation requiring correction", "Strong negative serial correlation in successive residuals", "Heteroskedasticity in the residual variance across observations"],
    answerIndex: 0,
    explanation: "The Durbin-Watson statistic approximates 2(1 - r), where r is the first-order residual autocorrelation. A value near 2 implies r near zero, meaning successive residuals are essentially uncorrelated. Values approaching 0 signal positive serial correlation and values approaching 4 signal negative. Durbin-Watson says nothing about the variance of the residuals, which is heteroskedasticity and requires the Breusch-Pagan test instead.",
  },
  {
    ...
    choices: ["Conditional heteroskedasticity in the regression residuals", "Omitting a relevant correlated variable", "Positive serial correlation among successive error terms", "Multicollinearity among the independent variables"],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: ["A linear trend model", "A random walk", "A log-linear trend model", "A seasonal model"],
    answerIndex: 2,
  ...
  },
  {
    ...
    choices: ["0.5", "0.32", "Undefined", "2.0"],
    answerIndex: 3,
  ...
  },
  {
    ...
    stem: "A stock price series is found to follow a random walk. What does this imply for an analyst attempting to model it?",
    choices: ["It is nonstationary and must be differenced before AR estimation", "It is stationary and can be modeled with an AR(1) directly", "It is mean-reverting toward a long-run equilibrium level", "Its variance is constant, so no transformation is required"],
    answerIndex: 0,
    explanation: "A random walk has a unit root: the coefficient on the lagged value equals one, so shocks are permanent and the variance grows without bound over time. That is nonstationarity, and AR estimation on such a series produces unreliable coefficients. First-differencing a random walk yields a stationary white-noise series suitable for modeling. Mean reversion is precisely what a random walk lacks, since it has no long-run level to revert toward.",
  },
  {
    ...
    choices: ["Durbin–Watson test", "Dickey–Fuller test", "Breusch–Pagan test", "Ordinary t-test on b₁"],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: ["No relationship at all", "A guaranteed unit root in the residuals", "A spurious but high R² and 'significant' coefficients", "Heteroskedastic but unbiased results"],
    answerIndex: 2,
  ...
  },
  {
    ...
    stem: "Two forecasting models fit the same in-sample data equally well. How should an analyst choose between them?",
    choices: ["Select the model with the higher in-sample R-squared statistic", "Select the model with more independent variables for greater coverage", "Compare the models' Durbin-Watson statistics and choose the higher", "Compare out-of-sample forecast accuracy using root mean squared error"],
    answerIndex: 3,
    explanation: "In-sample fit rewards complexity and is easily inflated by overfitting, which is exactly why two models can look equally good in sample and perform very differently out of it. Comparing RMSE on data the models never saw tests genuine predictive power. Higher R-squared and more variables both bias the choice toward the overfit model, and Durbin-Watson diagnoses serial correlation rather than ranking forecast quality.",
  },
  ...
  },
  {
    ...
    stem: "An analyst forecasts free cash flow to the firm and wants to arrive at the value of equity. What discount rate applies, and what adjustment follows?",
    choices: ["Discount FCFF at the cost of equity, giving equity value directly", "Discount FCFF at the WACC, giving equity value with no adjustment", "Discount FCFF at the WACC, then subtract the market value of debt", "Discount FCFF at the cost of debt, then add the value of equity"],
    answerIndex: 2,
    explanation: "FCFF is the cash available to all capital providers, so it must be discounted at the blended required return of all of them, the WACC. That produces enterprise value, from which the market value of debt is subtracted to reach equity value. FCFE, by contrast, is already net of debt service and is discounted at the cost of equity to give equity value directly. Mixing the two - FCFF at the cost of equity, or FCFF at WACC without subtracting debt - double-counts or omits the debtholders' claim.",
  },
  {
  ...
  },
  {
    ...
    choices: ["Residual income model", "Gordon growth DDM", "FCFE model", "Dividend yield"],
    answerIndex: 0,
  ...
  },
  {
    ...
    choices: ["Ignores the firm’s debt entirely", "Is unaffected by capital structure differences", "Always gives a higher value", "Relies on net income as the input"],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: ["Earnings are at a cyclical trough", "The firm pays no dividend", "Earnings are at a cyclical peak", "Growth is negative"],
    answerIndex: 2,
  ...
  },
  {
    ...
    stem: "In a two-stage DDM with three years of 15% growth followed by 4% perpetual growth at a 10% required return, the terminal value accounts for roughly 86% of total value. This implies that:",
    choices: ["The high-growth forecast drives most of the estimated value", "The model is unreliable and a multiples approach must be used", "The required return has little influence on the final estimate", "The valuation is highly sensitive to the terminal growth assumption"],
    answerIndex: 3,
    explanation: "When the terminal value dominates, small changes in the perpetual growth rate or the required return move the estimate substantially, because the terminal value denominator is (r - g) and both inputs sit in it. That is a call for careful sensitivity analysis, not a reason to abandon the model - the concentration in terminal value is a normal feature of DDM rather than a defect. The explicit high-growth years contribute the smaller share, and the required return matters more here, not less.",
  },

  {
    ...
    choices: ["The equity method", "Fair value through profit or loss", "Full consolidation", "Amortized cost"],
    answerIndex: 0,
  ...
  },
  {
    ...
    choices: ["Higher reported revenue and total assets", "Identical net income, but lower revenue and assets", "Lower reported net income for the period", "Higher non-controlling interest"],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: ["Net income", "Retained earnings directly", "The cash flow statement", "Other comprehensive income"],
    answerIndex: 3,
  ...
  },
  {
    ...
    choices: ["The contribution made", "The change in the projected benefit obligation", "Service cost plus interest cost", "The funded status"],
    answerIndex: 0,
  ...
  },
  {
    ...
    choices: ["The benefit obligation only", "Plan assets minus the benefit obligation", "Service cost plus interest cost", "Employer contributions for the year"],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: ["Increase reported pension expense", "Have no effect on the income statement", "Decrease pension expense, flattering operating income", "Increase the benefit obligation"],
    answerIndex: 2,
  ...
  },
  {
    ...
    choices: ["Service cost only", "Interest cost minus expected return", "The amount reported in P&L under US GAAP", "Employer contributions minus the change in funded status"],
    answerIndex: 3,
  ...
  },
  {
    ...
    choices: ["The bond's yield to maturity", "The spot rate for that cash flow's maturity", "The bond’s stated coupon rate", "The short-term risk-free rate"],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: ["Higher", "Equal", "Lower", "Always zero"],
    answerIndex: 2,
  ...
  },
  {
    ...
    choices: ["Raise the value of a callable bond", "Have no effect on either callable or putable bonds", "Lower the value of a putable bond", "Lower the value of a callable bond"],
    answerIndex: 3,
  ...
  },
  {
    ...
    choices: ["Positive convexity and higher effective duration", "Negative convexity and lower effective duration", "Zero effective duration across all rate levels", "The same convexity as a straight bond"],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: ["Zero", "Equal to the spot price", "Equal to the present value of the forward price", "Equal to the cost of carry"],
    answerIndex: 0,
  ...
  },
  {
    ...
    choices: ["The underlying price", "The strike price", "The volatility", "The risk-free rate"],
    answerIndex: 2,
  ...
  },
  {
    ...
    stem: "A fund holds 10,000 shares and wishes to hedge with call options having a delta of 0.50. How many contracts should be sold, and why?",
    choices: ["100 contracts sold, matching one contract per hundred shares held", "50 contracts sold, applying the delta directly to the share count", "200 contracts bought, to add exposure in the same direction", "200 contracts sold, since each option moves half as much as a share"],
    answerIndex: 3,
    explanation: "Delta measures the option's price sensitivity to a $1 move in the underlying, so a 0.50-delta call moves half as much as the stock. Hedging 10,000 shares therefore requires 10,000 / 0.50 = 20,000 option-equivalent shares, or 200 contracts of 100 shares each. One-for-one matching ignores delta entirely, and multiplying by delta rather than dividing moves the adjustment the wrong way. The options must be SOLD to offset a long stock position.",
  },

  {
    ...
    choices: ["1.2745", "1.2255", "1.2500", "1.3000"],
    answerIndex: 0,
  ...
  },
  {
    ...
    choices: ["Covered interest rate parity fails to hold", "Purchasing power parity holds exactly", "Uncovered interest rate parity fails to hold empirically", "Forward rates are unbiased predictors"],
    answerIndex: 2,
  ...
  },
  {
    ...
    choices: ["Appreciate", "Stay fixed", "Trade at a forward premium", "Depreciate"],
    answerIndex: 3,
  ...
  },
  {
    ...
    choices: ["Negative skew with fat tails", "Positive skew with thin tails", "A symmetric, normal distribution", "Zero variance"],
    answerIndex: 0,
  ...
  },
  {
    ...
    stem: "An analyst prefers APT over the CAPM for a multi-factor equity strategy. What is the strongest justification for that preference?",
    choices: ["APT is derived from the same equilibrium conditions but with more factors", "APT specifies which macroeconomic factors are priced in equilibrium", "APT eliminates the need to estimate factor sensitivities empirically", "APT requires no assumptions about the market portfolio or investor utility"],
    answerIndex: 3,
    explanation: "APT rests on a no-arbitrage argument rather than on CAPM's equilibrium assumptions about mean-variance investors and an observable market portfolio, and it accommodates multiple priced factors. That freedom from restrictive assumptions is its strength. Its corresponding weakness is that the theory does not tell you WHICH factors are priced - those must be chosen and their sensitivities estimated empirically, which is why the last two statements invert the model's actual properties.",
  },
  {
    ...
    choices: ["Momentum (WML)", "Liquidity", "Credit spread", "Inflation"],
    answerIndex: 0,
  ...
  },
  {
    ...
    choices: ["Factor sensitivities (betas), via regression on factor surprises", "Risk-free rate", "Number of factors, via principal-components analysis", "Factor returns, given attribute-based sensitivities"],
    answerIndex: 3,
  ...
  },
  {
    ...
    choices: ["The cost of financial distress", "Present value of the interest tax shield", "The cost of equity times debt", "Zero — value is unchanged"],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: ["Decrease EPS, because the reduction in share count lowers total earnings", "Leave EPS unchanged, since the transaction is purely financial", "Increase EPS only if funded with new debt", "Increase EPS, because the earnings yield exceeds the funding cost"],
    answerIndex: 3,
  ...
  },
  {
    ...
    choices: ["Decrease BVPS", "Increase BVPS", "Leave BVPS unchanged", "Eliminate BVPS"],
    answerIndex: 0,
  ...
  },
  {
    ...
    stem: "A board argues shareholders will accept a lower required return in exchange for higher current dividends. Which dividend theory is the board invoking, and what is the counterargument?",
    choices: ["Dividend irrelevance; critics reply that taxes and signaling do matter", "Bird-in-hand; critics reply dividends do not alter total firm risk", "Tax preference; critics reply many investors face no dividend tax", "Clientele effect; critics reply investors cannot select policies"],
    answerIndex: 1,
    explanation: "The bird-in-hand argument holds that investors value certain current dividends above uncertain future capital gains and therefore demand a lower return from high-payout firms. The standard rebuttal, following Miller and Modigliani, is that a dividend does not change the riskiness of the firm's underlying cash flows - it only changes the timing of their delivery, so the required return should be unaffected. The other theories describe different mechanisms entirely.",
  },

  {
    ...
    choices: ["DPI minus RVPI", "Committed capital divided by paid-in", "DPI plus RVPI", "Management fee plus carried interest"],
    answerIndex: 2,
  ...
  },
  {
    ...
    choices: ["Always positive and steadily rising", "Constant throughout the fund's life", "Highest at the fund’s inception date", "Negative early, turning positive as exits occur"],
    answerIndex: 3,
  ...
  },
  {
    ...
    choices: ["$7.2M", "$20.0M", "$14.4M", "$72.0M"],
    answerIndex: 1,
  ...
  },
  {
    ...
    stem: "A $10 million bond exposure carries a 3% probability of default and an expected recovery rate of 60%. What is the expected loss?",
    choices: ["$300,000, the full exposure at the default probability", "$120,000, reflecting a 40% loss given default", "$180,000, applying the recovery rate to the exposure", "$400,000, the loss given default applied to face value"],
    answerIndex: 1,
    explanation: "Loss given default is one minus the recovery rate: 1 - 0.60 = 40%. Expected loss multiplies all three components: $10,000,000 x 3% x 40% = $120,000. The $300,000 figure omits recovery entirely and assumes total loss on default. The $180,000 figure applies the 60% recovery rate where LGD belongs, inverting the relationship. The $400,000 figure ignores that default is only 3% likely.",
  },
  {
    ...
    choices: ["A risk-free bond", "A put option on the firm's debt", "A call option on the firm's assets", "A statistical hazard process"],
    answerIndex: 2,
  ...
  },
  {
    ...
    stem: "A credit analyst chooses a reduced-form model over a structural model. What does this choice imply about the modeling approach?",
    choices: ["Default is modeled as firm asset value falling below the debt threshold", "The model requires the firm's equity to be publicly traded and observable", "The model assumes recovery rates are always zero at the point of default", "Default is modeled statistically from observables, not from asset value"],
    answerIndex: 3,
    explanation: "Structural models treat default as an economic event: the firm's asset value falls below the face value of its debt, in the Merton option-theoretic framing. That requires estimating unobservable asset value and volatility, typically from traded equity. Reduced-form models instead treat default as a statistically modeled event driven by observable variables such as macro conditions and spreads, which is what allows them to be applied to firms without traded equity. Neither approach assumes zero recovery.",
  },
  {
    id: "cfa-l2cr-d5", examSlug: "cfa-l2", topicId: "fixed", topicName: "Fixed Income", difficulty: 2,
    stem: "An analyst values a corporate bond by discounting its cash flows at Treasury rates and arrives at $1,042. The credit valuation adjustment is $58. The bond's fair value is:",
    choices: ["$984, the risk-free value less the credit valuation adjustment", "$1,100, the risk-free value plus the credit valuation adjustment", "$1,042, since CVA affects the yield rather than the price", "$58, the credit valuation adjustment considered on its own"],
    answerIndex: 0,
    explanation: "The value of a credit-risky bond equals its value as if risk-free minus the CVA, the present value of expected credit losses: $1,042 - $58 = $984. Adding the CVA would imply the market pays a premium for default risk, which reverses the economics. CVA has a direct price effect rather than operating only through the quoted yield - the yield differential is the consequence of the price discount, not a substitute for it.",
  },
  {
    ...
    choices: ["Expected credit loss", "Changes in the benchmark risk-free rate", "A risk premium for credit uncertainty", "A liquidity premium"],
    answerIndex: 1,
...

  {
    ...
    choices: ["Firm B has higher earnings quality (more accruals)", "Both are identical because net income is the same", "Firm A has higher earnings quality (earnings backed by cash)", "Earnings quality cannot be assessed from this"],
    answerIndex: 2,
  ...
  },
  {
    ...
    choices: ["They are the same concept described using two different labels", "Reporting quality concerns sustainability; earnings quality concerns GAAP compliance", "Both refer strictly to the quality of reported operating cash flow", "Reporting quality is about trustworthy numbers; earnings quality, sustainability"],
    answerIndex: 3,
  ...
  },
  {
    ...
    choices: ["Recognizing revenue earlier than warranted", "Expensing a cost that could be capitalized", "Increasing bad-debt reserves", "Shortening depreciable asset lives"],
    answerIndex: 0,
  ...
  },
  {
    ...
    stem: "An analyst computes a Beneish M-score of -1.20 for a company under review. What does this suggest, and what should follow?",
    choices: ["It falls below the threshold, indicating the financial statements are clean", "It exceeds the -1.78 threshold, flagging manipulation risk for investigation", "It measures bankruptcy probability rather than manipulation risk", "It confirms manipulation occurred and restatement is therefore required"],
    answerIndex: 1,
    explanation: "The Beneish M-score combines eight variables into a probability that earnings have been manipulated, with scores above roughly -1.78 flagging elevated risk. At -1.20 the company crosses that threshold and warrants deeper investigation. Crucially, the model flags risk rather than proving misconduct - treating it as confirmation overstates what a statistical screen can establish. Bankruptcy probability is the Altman Z-score, a different model.",
  },
  {
    id: "cfa-l2qr-d5", examSlug: "cfa-l2", topicId: "fra", topicName: "Financial Reporting & Analysis", difficulty: 3,
    stem: "A company reports rising net income while operating cash flow declines for three consecutive years. This divergence is best interpreted as:",
    choices: ["A normal outcome of growth, since receivables rise alongside sales", "A signal of conservative accounting, since income exceeds cash", "A warning sign, since earnings are increasingly driven by accruals", "Irrelevant, since the two figures measure different periods"],
    answerIndex: 2,
    explanation: "A persistent and widening gap between accrual earnings and cash generation means income is increasingly composed of accruals rather than cash, and accruals are where management discretion lives. Sustained over three years, it is among the strongest earnings-quality warnings available from the statements alone. Growth can explain a single year's divergence but not a persistent one. Income exceeding cash is the opposite of conservatism, and both figures cover the same period.",
  },
  {
    ...
    choices: ["Outperform, because accruals signal growth", "Carry systematically higher credit ratings", "Pay materially larger dividends", "Underperform, since accrual earnings mean-revert"],
    answerIndex: 3,
